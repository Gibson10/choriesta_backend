  ...
  //get a user from the user token
   ...
   static async updateUser(req, res) {
         ...
         try {
            ...
            user.save();
            return res.send({ success: true, user: user, token: user.tokens[0].token });
         } catch (e) {
   ...
   //get all messages
	static async getMessages(req, res) {
         try {
            const user = req.user;
            const messages=  user.role === "choreowner"? await Messages.find({sender:user._id  }).populate('receiver'): await Messages.find({receiver:user._id }).populate('sender')
